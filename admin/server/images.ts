...
export const IMAGE_DIR = path.join(REPO_ROOT, 'public/assets/film/web');
...
/**
 * Picks a free filename under IMAGE_DIR for the given slug/extension,
 * treating `allowOverwritePath` (the entry's own current image, when
 * editing) as not a collision.
 */
export function resolveImagePath(
  ...
  while (true) {
    ...
    if (!existsSync(absPath) || absPath === allowOverwritePath) {
      return { absPath, imgPath: `/assets/film/web/${filename}` };
    }
    attempt += 1;
  }
}
