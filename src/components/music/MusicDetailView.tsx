import MusicPhotoCarousel from './MusicPhotoCarousel';
import type { FlatMusicProject } from '../../data/musicProjects';
import '../../pages/MusicDetailPage.css';

// The actual content of a music project's detail page, extracted out of
// MusicDetailPage the same way FilmDetailView was extracted out of
// FilmDetailPage — so the admin editor's live preview renders the exact
// same thing a visitor would see. MusicDetailPage still owns routing,
// data-fetching, and the back link.
//
// "Love Cycles" is a hand-built "coming soon" placeholder (a rotating SVG
// text orbit) keyed on its slug, not on any data field — same kind of
// special case as Film's CARD_SLUGS, kept here rather than invented as a
// new schema field for a one-off.
interface MusicDetailViewProps {
  project: FlatMusicProject;
}

function MusicDetailView({ project }: MusicDetailViewProps) {
  if (project.slug === 'love-cycles') {
    return (
          ...
          <div className="music-detail-orbit">
            ...
            >
              ...
              <text className="music-detail-orbit-text">
                <textPath href="#love-cycles-detail-circle" startOffset="0%" textLength="944" lengthAdjust="spacingAndGlyphs">
      ...
      <div className="film-detail-hero music-detail-hero">
        {project.imgPath ? (
          <div className="music-detail-art">
            <img src={project.imgPath} alt={project.title} loading="lazy" decoding="async" />
          </div>
        ) : null}
        <div className="film-detail-header">
          <h1 className="film-detail-title">{project.title}</h1>
          <p className="film-detail-meta">{[project.role, project.year].filter(Boolean).join(' · ') || 'Music'}</p>
        ...
        <section className="music-detail-media-section">
          <div className="music-detail-link-list">
            {project.links?.map((link) => (
              <a key={link.href} href={link.href} target="_blank" rel="noreferrer" className="music-detail-link-item">
                {link.label} →
              </a>
            ))}
            <a href={pdfHref} target="_blank" rel="noreferrer" className="music-detail-link-item">
